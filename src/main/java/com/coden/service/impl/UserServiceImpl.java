...
import com.coden.entity.vo.UserVO;
...
public class UserServiceImpl implements IUserService {
    ...
    @Override
    public BaseApiResult updateUserInfo(UserDTO userDTO) {
        String userId = userDTO.getId();
        String dtoPassword = userDTO.getPassword();
        RegistryUserDTO registryUserDTO = new RegistryUserDTO();
        registryUserDTO.setPassword(dtoPassword);
        String encodePassword = registryUserDTO.getEncodePassword();
        User user = mongoTemplate.findById(userId, User.class, COLLECTION_NAME);
        ...
        update.set("description",userDTO.getDescription());
        update.set("male",userDTO.isMale());
        update.set(UPDATE_TIME, new Date());
        UpdateResult updateResult = mongoTemplate.updateFirst(query, update, COLLECTION_NAME);
        long matchedCount = updateResult.getMatchedCount();
        if (matchedCount > 0) {
