...
import com.coden.entity.vo.DocWithCateVO;
...
public class FileServiceImpl implements IFileService {
    ...
    public BaseApiResult documentUpload(MultipartFile file, String userId, String username) throws AuthenticationException {
        ...
        List<String> availableSuffixList = com.google.common.collect.Lists
                .newArrayList("PDF","pdf", "docx", "doc","DOCX","DOC");
        try {
...
//        ObjectId objectId = gridFsTemplate.store()
    ...
    public List<FileDocument> listFilesByPage(int pageIndex, int pageSize) {
        Criteria criteria = Criteria.where("reviewing").is(false);
    ...
    @Override
    public BaseApiResult getMyUploadFilesList(String userId) {
        Criteria criteria = Criteria.where("reviewing").is(false).and("userId").is(userId);
        Query query = new Query().with(Sort.by(Sort.Direction.DESC, "uploadDate"));
        query.equals(userId);
        query.addCriteria(criteria);
        Field field = query.fields();
        field.exclude(CONTENT);
        List<FileDocument> myFiles = mongoTemplate.find(query, FileDocument.class, COLLECTION_NAME);
        if (myFiles == null){
            return BaseApiResult.error(303,"查询失败");
        }
        if (myFiles.size() == 0){
            return BaseApiResult.success();
        }
        return BaseApiResult.success(myFiles);
    }
    ...
    /**
     * @return java.util.List<DocumentVO>
    ...
    public DocumentVO convertDocument(DocumentVO documentVO, FileDocument fileDocument) {
        ...
        //命中的得分
        documentVO.setScore(fileDocument.getScore());
